...
def save_db(EPC, antenna_number):
    epc_time = time.strftime('%l:%M%p %Z on %b %d, %Y')
    # Tags = db.sel_db()
    # for Tag in Tags:
    ...
    db1 = reader_db()
    db1.insert_db(EPC, epc_time, antenna_number)

    print "Inserted EPC: {}, epc_time: {}".format(EPC, epc_time)
    # return True
